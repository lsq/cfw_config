...
async function parse_data() {
  try {
   ...
   const data = response.data 
    ...
    const tr = root.querySelector('.markdown-body > table:nth-child(24) > tbody:nth-child(2) > tr:nth-child(1)')
    ...
    // console.log(info)
    const jsonStr = '{"' + info.replace(/\s([^\s]*?：)/g, ',$1').replace(/：/g,'":"').replace(/,/g,'","').replace(/ /g,'') + '"}'
    // console.log(jsonStr)
