...
3. 读取fileYaml列表，直接为proxies列表，列表内容中的所有name前缀添加文件名，供后续合并使用
4.将步骤1-3中的所有proxies列表合并到步骤1中记录为main字段的proxies中，
5. 应用fix字段中的base字段内容到合并后的main 主配置中
6. 应用rules字段内容到main主配置中
7. 最后保存main配置为config.yaml
