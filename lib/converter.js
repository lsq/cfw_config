...
exports.linkToClash = linkToClash;
exports.tryDecodeBase64SubscriptionLinks = tryDecodeBase64SubscriptionLinks;
exports.clashToLink = clashToLink;
exports.default = parseUri;
exports.generateUri = generateUri;
const utils_1 = require("./utils");
const jsyaml_1 = require("./jsyaml");
const pyyaml_1 = require("./pyyaml");
const js_yaml_1 = require("js-yaml");
let currentParser = 'js';
const CIPHER_VALUES = [
    'none',
    'auto',
    'dummy',
...
// ====================== 正向：链接 → Clash ======================
function linkToClash(links, mode = 'proxies') {
...
function linksToClashNodes(links) {
    return links
        .map((link) => {
        try {
...
function tryDecodeBase64SubscriptionLinks(links) {
...
// ====================== 反向：Clash → 链接 ======================
async function clashToLink(yamlText) {
    try {
        ...
        let config;
        if (currentParser === 'js') {
            config = (0, jsyaml_1.parseJsYaml)(yamlText);
            if (!config) {
                return {
                    success: false,
            ...
            config = await (0, pyyaml_1.parsePyYaml)(yamlText);
        }
        ...
        const candidates = [
            ...
            ...(() => {
                ...
                if (providers &&
                    ...
                    return Object.values(providers).flatMap((p) => p?.proxies || p?.payload || []);
                }
                return [];
            })(),
            // 直接就是节点数组的情况
        ...
        const links = proxies.map((node) => generateUri(node)).filter(Boolean);
        return {
            success: true,
            data: links.join('\n'),
        };
...
function decodeBase64Strict(str) {
    try {
        const binary = atob(str);
        try {
            const bytes = Uint8Array.from(binary, (c) => c.charCodeAt(0));
...
function isCipherType(value) {
    return CIPHER_VALUES.includes(value);
}
function getCipher(str) {
    if (str !== undefined && isCipherType(str)) {
        return str; // ✅ 此时 TS 知道 str 是 CipherType
    }
    return 'auto'; // 默认值
}
function URI_SS(line) {
    ...
    if (!proxy.name)
        proxy.name = `SS ${proxy.server}:${proxy.port}`;
    return proxy;
}
...
function URI_VMESS(line) {
    ...
    if (/=\s*vmess/.test(content)) {
        ...
        else if (['http'].includes(params.net) ||
...
function URI_VLESS(line) {
    ...
    const server = serverRaw.startsWith('[') && serverRaw.endsWith(']')
        ? serverRaw.slice(1, -1)
        : serverRaw;
    let uuid = uuidRaw;
    if (isShadowrocket) {
...
function URI_Trojan(line) {
    ...
    const [, passwordRaw, serverRaw, , port, , addons = '', nameRaw] = /^(.*?)@(.*?)(:(\d+))?\/?(\?(.*?))?(?:#(.*?))?$/.exec(line) || [];
    // Strip IPv6 brackets: "[2001:db8::1]" → "2001:db8::1"
    const server = serverRaw?.startsWith('[') && serverRaw?.endsWith(']')
        ? serverRaw.slice(1, -1)
        : serverRaw;
...
function URI_Hysteria2(line) {
    ...
    const proxy = {
        ...
        ...((params.get('insecure') === '1' ||
            ...
            /true/i.test(params.get('skip-cert-verify') || '')) && {
...
function URI_Hysteria(line) {
    ...
    if (!proxy.sni && peerFallback)
        proxy.sni = peerFallback;
    if (!proxy.protocol) {
        proxy.protocol = 'udp';
    }
    return proxy;
...
function URI_TUIC(line) {
    ...
    const [, uuid, passwordRaw, server, , port, , addons = '', nameRaw] = /^(.*?):(.*?)@(.*?)(:(\d+))?\/?(\?(.*?))?(?:#(.*?))?$/.exec(line) || [];
    let portNum = Number.parseInt(`${port}`, 10);
    if (isNaN(portNum)) {
        portNum = 443;
    }
...
function URI_Wireguard(line) {
    ...
    const [, , privateKeyRaw, server, , port, , addons = '', nameRaw] = /^((.*?)@)?(.*?)(:(\d+))?\/?(\?(.*?))?(?:#(.*?))?$/.exec(line);
    let portNum = Number.parseInt(`${port}`, 10);
    if (isNaN(portNum)) {
        portNum = 443;
    }
...
function URI_HTTP(line) {
    ...
    const [, , authRaw, server, , port, , addons = '', nameRaw] = /^((.*?)@)?(.*?)(:(\d+))?\/?(\?(.*?))?(?:#(.*?))?$/.exec(line);
    let portNum = Number.parseInt(`${port}`, 10);
    if (isNaN(portNum)) {
        portNum = 443;
    }
...
function generateClashNode(node) {
    const fallbackName = node.name ||
        `${node.type?.toUpperCase() || 'Node'} ${node.server}:${node.port}`;
    const clashNode = {
        name: fallbackName,
        type: node.type,
        server: node.server ? (0, utils_1.punycodeDomain)(node.server) : node.server,
        port: node.port,
    };
    ...
    for (const field of Object.keys(node)) {
        if (handled.has(field) || field.startsWith('_'))
            continue;
    ...
    // 生成带 - 的完整节点字符串
    const yamlLines = (0, js_yaml_1.dump)(cleaned, { indent: 2 }).trim().split('\n');
...
// ====================== 生成原始链接（完整支持所有协议）=====================
function generateUri(node) {
    const name = encodeURIComponent(node.name || 'Node');
    const server = node.server ? (0, utils_1.punycodeDomain)(node.server) : node.server;
    ...
    switch (node.type) {
        ...
        case 'vless':
            ...
            if (node.tls || node['reality-opts']) {
                ...
                if (Array.isArray(node.alpn) && node.alpn.length)
        ...
        case 'anytls':
            ...
            if (node.sni)
                aParams.set('sni', node.sni);
            if (Array.isArray(node.alpn) && node.alpn.length)
                aParams.set('alpn', node.alpn.join(','));
            if (node['client-fingerprint'])
                aParams.set('client-fingerprint', node['client-fingerprint']);
            if (node['skip-cert-verify'])
                aParams.set('allowInsecure', '1');
            if (node.udp)
                aParams.set('udp', '1');
            ...
            if (node['idle-session-timeout'])
                aParams.set('idle-session-timeout', String(node['idle-session-timeout']));
            if (node['min-idle-session'])
                aParams.set('min-idle-session', String(node['min-idle-session']));
            return (anytls +
        ...
        case 'hysteria': {
            ...
            if (node.protocol)
                hParams.set('protocol', node.protocol);
            if (node['auth-str'])
                hParams.set('auth', node['auth-str']);
            if (node.sni)
                hParams.set('sni', node.sni);
            if (node.up)
                hParams.set('upmbps', String(node.up));
            if (node.down)
                hParams.set('downmbps', String(node.down));
            if (Array.isArray(node.alpn) && node.alpn.length)
                hParams.set('alpn', node.alpn.join(','));
            if (node.obfs)
                hParams.set('obfs', node.obfs);
            if (node.ports)
                hParams.set('mport', String(node.ports));
            if (node['skip-cert-verify'])
                hParams.set('insecure', '1');
        ...
        case 'wireguard': {
            ...
            if (node['public-key'])
                wgParams.set('public-key', node['public-key']);
            const wgAddrs = [node.ip, node.ipv6].filter(Boolean);
            if (wgAddrs.length)
                wgParams.set('address', wgAddrs.join(','));
            if (Array.isArray(node['allowed-ips']) && node['allowed-ips'].length)
                wgParams.set('allowed-ips', node['allowed-ips'].join(','));
            if (node['pre-shared-key'])
                wgParams.set('pre-shared-key', node['pre-shared-key']);
            if (Array.isArray(node.reserved) && node.reserved.length === 3)
                wgParams.set('reserved', node.reserved.join(','));
            if (node.mtu)
                wgParams.set('mtu', String(node.mtu));
            if (Array.isArray(node.dns) && node.dns.length)
                wgParams.set('dns', node.dns.join(','));
        ...
        case 'http': {
            ...
            if (node.tls)
                httpParams.set('tls', '1');
            if (node.fingerprint)
                httpParams.set('fingerprint', node.fingerprint);
            if (node['skip-cert-verify'])
                httpParams.set('skip-cert-verify', '1');
            if (node['ip-version'])
                httpParams.set('ip-version', node['ip-version']);
            return (http +
                (httpParams.toString() ? '?' + httpParams.toString() : '') +
                `#${name}`);
        }
        case 'socks5': {
            const hasAuth = !!(node.username || node.password);
            ...
            if (node.tls)
                socksParams.set('tls', '1');
            if (node.fingerprint)
                socksParams.set('fingerprint', node.fingerprint);
            if (node['skip-cert-verify'])
                socksParams.set('skip-cert-verify', '1');
            if (node.udp)
                socksParams.set('udp', '1');
            if (node['ip-version'])
                socksParams.set('ip-version', node['ip-version']);
            return (socks +
                (socksParams.toString() ? '?' + socksParams.toString() : '') +
                `#${name}`);
        }
        default:
            return '';
