...
const xml = '<books><book><title lang="en">XPath指南</title></book><book><title lang="en">XxPath指南</title></book></books>'
...
const str = '节点：92.118.205.61 端口：22222 密码： dongtaiwang.com  加密方式：aes-256-gcm'

console.log('{' + str.replace(/\s([^\s]*?)：/g, ',$1:')+ '}')
jsonStr = '{"' + str.replace(/\s([^\s]*?：)/g, ',$1').replace(/：/g,'":"').replace(/,/g,'","')+ '"}'
...
const cNode = xpath.select("//code[preceding::*[self::strong and contains(text(),'SS节点')]]",newDoc)
