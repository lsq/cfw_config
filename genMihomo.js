...
const {exportData, mihomoConfig, restartMihomo, createSymlink} = require('./get_newpac')
const newline = /\r?\n/;
const trimText = /机场推荐：/
 ...
 * @param {number} [baseIndent=2] - 子内容的缩进空格数（默认 2）
...
function renderProxyProvider(name, config, template) {
  const lines = [`  ${name}:`];
  if (config.type === 'http' && template.providersAnchorUrl) { // 你可加个标志
  ...
  if (config.path) lines.push(`    path: ${template.providersDir ? (template.providersDir + '/') : './'}${config.path}`);
  ...
  'additional-prefix': `${name}${prefix}`
      ...
      lines.push(`${objectToYamlSubBlock("    override", overrideWithPrefix, 6)}`);
  }
  return lines.join('\n');
}
...
async function processLocalFiles(files, parseFn) {
    ...
    const tasks = files.map(async (file) => {
        ...
        return {name: prefix, proxies: proxies.map(p => ({
            ...p,
    ...
    for (const provider of providers) {
        for (const [index, url] of provider.url.entries()) {
            tasks.push((async () => {
                ...
                const rawContent = await downloadFile(url,`./downloads/${localFilename}`)
                ...
                if (provider.main) {
                    ...
                    try {
                        ...
                        console.warn(`⚠️ Provider [${provider.name} - ${localFilename}] 解析失败:`, e.message);
                    }
                    ...
                    proxies = proxies.map(p => ({ ...p, name: `${provider.name}${index > 0 ? index : ''}|${rmKeywords(p.name, trimText)}` }));
                    return { isMain: false, proxies, mainConfig: null, name: `${provider.name}|${index}` };
                }
            })());
        }
    }
...
async function methodOne(config, mihomoCfg) {
    const [providerResult, fileNameProxies, fileYamlProxies] = await Promise.all([
        ...
        processLocalFiles(config.fileName || [], (content) => linkToConfig(content)),
    ...
    const providersStrategy = config?.['groups-strategy'].providers || {type: 'load-balance', interval: 300, lazy: true, strategy: 'consistent-hashing'}

    mainConfig['proxy-groups'] = [
        ...(mainConfig['proxy-groups'] || []),
        ...allProxyGroup.map(itm => ({name: itm.name, ...providersStrategy, proxies: itm.proxies.map(i=>i.name)})),
        ...fileNameProxies.map(itm =>({name:  itm.name, type: 'select', proxies: itm.proxies.map(i=>i.name)})),
        ...fileYamlProxies.map(itm =>({name:  itm.name, type: 'select', proxies: itm.proxies.map(i=>i.name)}))
    ];
...
async function methodTwo(config, mihomoCfg) {
    ...
    const proxyProvidersFromRemote = await processProvidersForMethod2(config.providers);
    ...
    const allLocalFiles = [...(config.fileName || []), ...(config.fileYaml || [])];
    const mihomoCfgHome = path.dirname(path.resolve(mihomoCfg))
    await createSymlink(path.resolve('downloads') ,path.join(mihomoCfgHome, template.providersDir))
    ...
    for (const file of allLocalFiles) {
        ...
        const providerName = (path.parse(file).name === 'default' ? 'back' : path.parse(file).name);
        yamlStr += renderProxyProvider(providerName, {type: 'file', path: destPath}, template) + '\n';
    }
